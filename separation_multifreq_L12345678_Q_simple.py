    ...
    if mode == 'classic_bias':
        ...
        for i in range(n_batch):
            ...
            for j in range(nb_chunks):
                ...
                batch_COEFFS[:,:,indices] = coeffs_chunk.type(dtype=ref_type) - wph_op.apply(x.unsqueeze(1).expand(contamination_batch[:,i].size()), norm=None, pbc=pbc)[:,:,indices].type(dtype=ref_type)
