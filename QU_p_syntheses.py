...
def objective(x):
    ...
    wph_op.clear_normalization()
    wph_op.apply(np.sqrt(x_target[0]**2+x_target[1]**2)/I, norm="auto", pbc=pbc)
    x_curr_p, nb_chunks = wph_op.preconfigure(torch.sqrt(x_curr[0]**2+x_curr[1]**2)/torch.from_numpy(I).to(device), requires_grad=True, pbc=pbc)
    for i in range(nb_chunks):
...
np.save(str(n_syn)+'_QU_pP_synthesis_'+str(freqs[freq])+'.npy',x_return)
