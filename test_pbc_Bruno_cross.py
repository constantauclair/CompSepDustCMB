...
file_name="test_pbc="+str(pbc)+"_dn="+str(dn)+"_logT_HM_Bruno.npy"
...
def compute_loss(x,coeffs_target,std,mask,cross):
    ...
    for j in range(Mn):
        u_noisy, nb_chunks = wph_op.preconfigure([x + torch.from_numpy(n_HM1[j]).to(device),x + torch.from_numpy(n_HM2[j]).to(device)], requires_grad=True, pbc=pbc, cross=cross)
...
def objective1(x):
    ...
    L = compute_loss(u,coeffs_target_L1,std_L1,mask_L1,cross=True) # Compute the loss
...
if __name__ == "__main__":
    ...
    for i in range(n_step1):
        ...
        # L1
        _, std_L1 = compute_bias_std_L1(s_tilde, s_tilde, n_HM1_batch, n_HM2_batch)
        print("Std =",std_L1)
