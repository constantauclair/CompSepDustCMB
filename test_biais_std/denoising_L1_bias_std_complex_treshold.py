...
def objective2(x):
    ...
    for i in range(nb_chunks):
        ...
        loss = torch.sum(torch.abs(torch.where(relevant_imaginary_coeffs[indices]==1,(torch.imag(coeffs_chunk) - coeffs_target[1][indices]) / std[1][indices],coeffs_chunk*0) ** 2))
