...
Bruno_result = np.load('separation_IQU_B_353_3steps_30iters.npy')[[2,6]].astype(np.float32)
...
def compute_bias_std_L123(u_A, u_B, conta_A, conta_B):
...
def compute_bias_std_L45(u_A, conta_A):
...
def compute_mask(x,std,cross=False,thresh=2.0):
    coeffs = wph_op.apply(x,norm=None,pbc=pbc,cross=cross)
    mask_real = torch.abs(torch.real(coeffs)/std[0].to(device)) > thresh
    mask_imag = torch.abs(torch.imag(coeffs)/std[1].to(device)) > thresh
...
def compute_L123(x_A,x_B,coeffs_target,std,mask):
    ...
    u, nb_chunks = wph_op.preconfigure([x_A,x_B], requires_grad=True, pbc=pbc, cross=True, mem_chunk_factor=50, mem_chunk_factor_grad=80)
...
def compute_L45(x,coeffs_target,std,mask):
    coeffs_target = coeffs_target.to(device)
    std = std.to(device)
    mask = mask.to(device)
    loss_tot = torch.zeros(1)
    u, nb_chunks = wph_op.preconfigure([x,torch.from_numpy(I).to(device)], requires_grad=True, pbc=pbc, cross=True, mem_chunk_factor=50, mem_chunk_factor_grad=80)
    for i in range(nb_chunks):
        coeffs_chunk, indices = wph_op.apply(u, i, norm=None, ret_indices=True, pbc=pbc, cross=True)
        loss = ( torch.sum(torch.abs( (torch.real(coeffs_chunk)[mask[0,indices]] - coeffs_target[0][indices][mask[0,indices]]) / std[0][indices][mask[0,indices]] ) ** 2) + torch.sum(torch.abs( (torch.imag(coeffs_chunk)[mask[1,indices]] - coeffs_target[1][indices][mask[1,indices]]) / std[1][indices][mask[1,indices]] ) ** 2) ) / ( mask[0].sum() + mask[1].sum() ) / Mn
        loss.backward(retain_graph=True)
...
def compute_L67(x,obs,coeffs_target,std,mask):
    coeffs_target = coeffs_target.to(device)
    std = std.to(device)
    mask = mask.to(device)
    loss_tot = torch.zeros(1)
    u, nb_chunks = wph_op.preconfigure(torch.from_numpy(obs).to(device) - x, requires_grad=True, pbc=pbc, cross=False, mem_chunk_factor=50, mem_chunk_factor_grad=80)
    for i in range(nb_chunks):
        coeffs_chunk, indices = wph_op.apply(u, i, norm=None, ret_indices=True, pbc=pbc, cross=False)
...
if __name__ == "__main__":
    ...
    # s_tilde0_for_bias_std = Bruno_result
    # # L6
    # print('Preparing L6...')
    ...
    #     s_tilde0_for_bias_std = torch.from_numpy(s_tilde0_for_bias_std).to(device) # Initialization of the map
    #     # L1
    #     print('Preparing L1...')
    #     bias_L1, std_L1 = compute_bias_std_L123(s_tilde0_for_bias_std[0], s_tilde0_for_bias_std[0], cn_Q_HM1_batch, cn_Q_HM2_batch)
    ...
    #     mask_L1 = compute_mask([torch.from_numpy(d_Q_HM1).to(device),torch.from_numpy(d_Q_HM2).to(device)], std_L1, cross=True)
    #     print('L1 prepared !')
    #     # L2
    #     print('Preparing L2...')
    #     bias_L2, std_L2 = compute_bias_std_L123(s_tilde0_for_bias_std[1], s_tilde0_for_bias_std[1], cn_U_HM1_batch, cn_U_HM2_batch)
    ...
    #     mask_L2 = compute_mask([torch.from_numpy(d_U_HM1).to(device),torch.from_numpy(d_U_HM2).to(device)], std_L2, cross=True)
    #     print('L2 prepared !')
    #     # L3
    #     print('Preparing L3..')
    #     bias_L3, std_L3 = compute_bias_std_L123(s_tilde0_for_bias_std[0], s_tilde0_for_bias_std[1], cn_Q_FM_batch, cn_U_FM_batch)
    ...
    #     mask_L3 = compute_mask([torch.from_numpy(d_Q_FM).to(device),torch.from_numpy(d_U_FM).to(device)], std_L3, cross=True)
    #     print('L3 prepared !')
    #     # L4
    #     print('Preparing L4...')
    #     bias_L4, std_L4 = compute_bias_std_L45(s_tilde0_for_bias_std[0], cn_Q_FM_batch)
    ...
    #     mask_L4 = compute_mask([torch.from_numpy(d_Q_FM).to(device),torch.from_numpy(I).to(device)], std_L4, cross=True)
    #     print('L4 prepared !')
    #     # L5
    #     print('Preparing L5...')
    #     bias_L5, std_L5 = compute_bias_std_L45(s_tilde0_for_bias_std[1], cn_U_FM_batch)
    ...
    #     mask_L5 = compute_mask([torch.from_numpy(d_U_FM).to(device),torch.from_numpy(I).to(device)], std_L5, cross=True)
    #     print('L5 prepared !')
    #     # Minimization
    ...
    #     s_tilde0_for_bias_std = s_tilde0
    ...
    eval_cnt = 0
    s_tilde = np.array([d_Q_FM,d_U_FM])
    s_tilde_for_bias_std = Bruno_result
    # L6
    print('Preparing L6...')
    wph_op.load_model(wph_model)
    coeffs_target_L6, std_L6 = compute_mean_std_L67(cn_Q_FM_batch)
    ...
    for i in range(n_step):
        ...
        print('Preparing L1...')
        wph_op.load_model(wph_model_cross)
        bias_L1, std_L1 = compute_bias_std_L123(s_tilde_for_bias_std[0], s_tilde_for_bias_std[0], cn_Q_HM1_batch, cn_Q_HM2_batch)
        coeffs_L1 = wph_op.apply([torch.from_numpy(d_Q_HM1).to(device),torch.from_numpy(d_Q_HM2).to(device)], norm=None, pbc=pbc, cross=True)
        coeffs_target_L1 = torch.cat((torch.unsqueeze(torch.real(coeffs_L1)-bias_L1[0],dim=0),torch.unsqueeze(torch.imag(coeffs_L1)-bias_L1[1],dim=0)))
        mask_L1 = compute_mask([torch.from_numpy(d_Q_HM1).to(device),torch.from_numpy(d_Q_HM2).to(device)], std_L1, cross=True)
        print('L1 prepared !')
        ...
        print('Preparing L2...')
        wph_op.load_model(wph_model_cross)
        bias_L2, std_L2 = compute_bias_std_L123(s_tilde_for_bias_std[1], s_tilde_for_bias_std[1], cn_U_HM1_batch, cn_U_HM2_batch)
        coeffs_L2 = wph_op.apply([torch.from_numpy(d_U_HM1).to(device),torch.from_numpy(d_U_HM2).to(device)], norm=None, pbc=pbc, cross=True)
        coeffs_target_L2 = torch.cat((torch.unsqueeze(torch.real(coeffs_L2)-bias_L2[0],dim=0),torch.unsqueeze(torch.imag(coeffs_L2)-bias_L2[1],dim=0)))
        mask_L2 = compute_mask([torch.from_numpy(d_U_HM1).to(device),torch.from_numpy(d_U_HM2).to(device)], std_L2, cross=True)
        print('L2 prepared !')
        ...
        print('Preparing L3...')
        wph_op.load_model(wph_model_cross)
        bias_L3, std_L3 = compute_bias_std_L123(s_tilde_for_bias_std[0], s_tilde_for_bias_std[1], cn_Q_FM_batch, cn_U_FM_batch)
        coeffs_L3 = wph_op.apply([torch.from_numpy(d_Q_FM).to(device),torch.from_numpy(d_U_FM).to(device)], norm=None, pbc=pbc, cross=True)
        coeffs_target_L3 = torch.cat((torch.unsqueeze(torch.real(coeffs_L3)-bias_L3[0],dim=0),torch.unsqueeze(torch.imag(coeffs_L3)-bias_L3[1],dim=0)))
        mask_L3 = compute_mask([torch.from_numpy(d_Q_FM).to(device),torch.from_numpy(d_U_FM).to(device)], std_L3, cross=True)
        print('L3 prepared !')
        ...
        print('Preparing L4...')
        wph_op.load_model(wph_model_cross)
        bias_L4, std_L4 = compute_bias_std_L45(s_tilde_for_bias_std[0], cn_Q_FM_batch)
        coeffs_L4 = wph_op.apply([torch.from_numpy(d_Q_FM).to(device),torch.from_numpy(I).to(device)], norm=None, pbc=pbc, cross=True)
        coeffs_target_L4 = torch.cat((torch.unsqueeze(torch.real(coeffs_L4)-bias_L4[0],dim=0),torch.unsqueeze(torch.imag(coeffs_L4)-bias_L4[1],dim=0)))
        mask_L4 = compute_mask([torch.from_numpy(d_Q_FM).to(device),torch.from_numpy(I).to(device)], std_L4, cross=True)
        print('L4 prepared !')
        ...
        print('Preparing L5...')
        wph_op.load_model(wph_model_cross)
        bias_L5, std_L5 = compute_bias_std_L45(s_tilde_for_bias_std[1], cn_U_FM_batch)
        coeffs_L5 = wph_op.apply([torch.from_numpy(d_U_FM).to(device),torch.from_numpy(I).to(device)], norm=None, pbc=pbc, cross=True)
        coeffs_target_L5 = torch.cat((torch.unsqueeze(torch.real(coeffs_L5)-bias_L5[0],dim=0),torch.unsqueeze(torch.imag(coeffs_L5)-bias_L5[1],dim=0)))
        mask_L5 = compute_mask([torch.from_numpy(d_U_FM).to(device),torch.from_numpy(I).to(device)], std_L5, cross=True)
        print('L5 prepared !')
