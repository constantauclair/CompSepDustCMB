...
file_name="separation_IQU_B_fast_"+str(freqs[freq])+"_Mn10_3steps_30iters.npy"
...
def compute_mask(x,cross=False):
...
if __name__ == "__main__":
    ...
    mask_L6 = compute_mask(cn_Q_FM_batch[0,0])
    print('L6 prepared !')
    # L7
    print('Preparing L7...')
    coeffs_target_L7, std_L7 = compute_mean_std_L67(cn_U_FM_batch)
    mask_L7 = compute_mask(cn_U_FM_batch[0,0])
    print('L7 prepared !')
    for i in range(n_step):
        ...
        mask_L1 = compute_mask([torch.from_numpy(d_Q_HM1).to(device),torch.from_numpy(d_Q_HM2).to(device)], cross=True)
        print('L1 prepared !')
        # L2
        print('Preparing L2...')
        ...
        mask_L2 = compute_mask([torch.from_numpy(d_U_HM1).to(device),torch.from_numpy(d_U_HM2).to(device)], cross=True)
        print('L2 prepared !')
        # L3
        print('Preparing L3..')
        ...
        mask_L3 = compute_mask([torch.from_numpy(d_Q_FM).to(device),torch.from_numpy(d_U_FM).to(device)], cross=True)
        print('L3 prepared !')
        # L4
        print('Preparing L4...')
        ...
        mask_L4 = compute_mask([torch.from_numpy(d_Q_FM).to(device),torch.from_numpy(I).to(device)], cross=True)
        print('L4 prepared !')
        # L5
        print('Preparing L5...')
        ...
        mask_L5 = compute_mask([torch.from_numpy(d_U_FM).to(device),torch.from_numpy(I).to(device)], cross=True)
        print('L5 prepared !')
        # Minimization
